...
function resolveTrayIcon() {
  try {
    // Packaged: prefer the app icon shipped via build resources.
    // Dev: also works because electron/build/icon.ico is generated by prep-assets.
    const candidateIco = path.join(__dirname, 'build', 'icon.ico');
    if (fs.existsSync(candidateIco)) {
      const img = nativeImage.createFromPath(candidateIco);
...
function setupTray() {
  ...
  try {
    ...
    tray.setToolTip('SÖNDBÖUND');

    const buildTrayMenu = () => Menu.buildFromTemplate([
      {
        label: (mainWindow && !mainWindow.isDestroyed() && mainWindow.isVisible()) ? 'Hide' : 'Show',
        click: () => {
    ...
    tray.on('right-click', () => {
      try { tray.popUpContextMenu(buildTrayMenu()); } catch {}
    });
    tray.on('click', () => {
    ...
    logLine('[tray] enabled');
  } catch (e) {
    logLine('[tray] setup failed: ' + (e && e.stack ? e.stack : e));
  }
}
...
async function createWindow() {
  ...
  // If tray is enabled, closing the window keeps the app running.
