...
# Desktop window (embedded webview)
python -m soundgen.desktop
```
...
- Copilot builds a small **on-disk local index** (for faster, richer context retrieval). You can rebuild it from the UI (“Rebuild Copilot index”) or disable it with `SOUNDGEN_AI_INDEX=0`.
